...
local TrainerPoints = {
    ["Rock Trainer"] = Vector3.new(-221.12864685058594, 825.3444213867188, 1354.0791015625),
    ["Water Trainer"] = Vector3.new(537.2061767578125, 821.2026977539062, 769.544921875),
    ["Beast Trainer"] = Vector3.new(-46.305755615234375, 821.6522827148438, 184.3372802734375),
    ["Thunder Trainer"] = Vector3.new(-2011.1715087890625, 830.0028686523438, -1043.781982421875),
    ...
    ["Love Trainer"] = Vector3.new(-1158.8599853515625, 885.0248413085938, 854.0323486328125),
    ["Mist Trainer"] = Vector3.new(-234.3750762939453, 820.4998779296875, -77.05122375488281),
}
...
-- Acquisition buttons. The addon below is installed AFTER the original hub finishes.
...
local TrainerOrder = {
    ...
    "Rat Trainer", "Love Trainer", "Mist Trainer"
}
local trainerY = 42
...
local SprintBypassEnabled = false
local SprintBypassConnections = {}
local AntiFallDmgEnabled = false
local AntiFallDmgConnection = nil
...
local function FindRunButton()
    local player = Players.LocalPlayer
    local pg = player:FindFirstChildOfClass("PlayerGui")
    if not pg then return nil end

    local mobileGui = pg:FindFirstChild("MobileGui")
    if not mobileGui then return nil end

    local frame = mobileGui:FindFirstChild("Frame")
    if not frame then return nil end

    local run = frame:FindFirstChild("run")
    if run and (run:IsA("ImageButton") or run:IsA("TextButton")) then
        return run
    end
    return nil
end

local function ResetInjury()
    local player = Players.LocalPlayer
    local injury = player:FindFirstChild("Injury")
    if not injury then return end

    for _, obj in ipairs(injury:GetDescendants()) do
...
local function SetupSprintStates(char)
    local states = char:FindFirstChild("States") or char:WaitForChild("States", 10)
    if not states then return end

    local connection = states.ChildAdded:Connect(function(child)
        if not SprintBypassEnabled or not child or not child.Parent then return end
        ...
        or n:find("bind") then
            task.defer(function()
                if SprintBypassEnabled and child and child.Parent then
    ...
    table.insert(SprintBypassConnections, connection)
end

local function StartSprintBypass()
    if SprintBypassEnabled then return end
    SprintBypassEnabled = true

    local player = Players.LocalPlayer

    if player.Character then
        SetupSprintStates(player.Character)
    end

    table.insert(SprintBypassConnections, player.CharacterAdded:Connect(function(char)
        if not SprintBypassEnabled then return end
        task.wait(0.5)
        if SprintBypassEnabled then
            SetupSprintStates(char)
        end
    end))

    task.spawn(function()
        while SprintBypassEnabled do
            task.wait(0.2)
            local runButton = FindRunButton()
            if runButton then
                pcall(function() runButton:Activate() end)
            end
        end
    end)
    ...
    print("[SPRINT BYPASS] Active")
end

local function StopSprintBypass()
    SprintBypassEnabled = false
    DisconnectConnections(SprintBypassConnections)
    print("[SPRINT BYPASS] Disabled")
end

local function StartAntiFallDmg()
    if AntiFallDmgEnabled then return end
    AntiFallDmgEnabled = true

    AntiFallDmgConnection = RunService.Heartbeat:Connect(function()
        if not AntiFallDmgEnabled then return end
    ...
    print("[ANTI FALL DMG] Active")
end

local function StopAntiFallDmg()
    AntiFallDmgEnabled = false

    if AntiFallDmgConnection then
        pcall(function() AntiFallDmgConnection:Disconnect() end)
        AntiFallDmgConnection = nil
    end

    print("[ANTI FALL DMG] Disabled")
end
...
-- Ice Shard uses the fixed trainer location instead of searching Workspace.
-- This is streaming-friendly: only the known Pure Ice Man path is accessed.
local function GetIceShardTrainerPosition()
    local ok, result = pcall(function()
        local ChatNPCs = workspace:WaitForChild("chatnpcs", 5)
        local PureIceMan = ChatNPCs:WaitForChild("Pure Ice Man", 5)
        local Chat = PureIceMan:WaitForChild("Chat", 5)
        local Learn = Chat:WaitForChild("Learn", 5)
        local Trainer = Learn:WaitForChild("IceShardTrainer", 5)

        if Trainer:IsA("BasePart") then
            return Trainer.Position
        elseif Trainer:IsA("Model") then
            return Trainer:GetPivot().Position
        end

        local Parent = Trainer.Parent
        while Parent and Parent ~= PureIceMan do
        ...
        return PureIceMan:GetPivot().Position
    end)
...
IceTweenButton.MouseButton1Click:Connect(function()
    ...
    -- Streaming-safe fixed paths only. Try these known locations:
    -- Workspace > chatnpcs > Pure Ice Man > ActiveIceShard
    -- Workspace > chatnpcs > Pure Ice Man > Chat > ActiveIceShard
    -- Workspace > chatnpcs > Pure Ice Man > Chat > Learn > ActiveIceShard
    local ChatNPCs = workspace:FindFirstChild("chatnpcs")
    local PureIceMan = ChatNPCs and ChatNPCs:FindFirstChild("Pure Ice Man")
    local Chat = PureIceMan and PureIceMan:FindFirstChild("Chat")
    local Learn = Chat and Chat:FindFirstChild("Learn")

    local Shard = PureIceMan and PureIceMan:FindFirstChild("ActiveIceShard")
    if not Shard and Chat then
        Shard = Chat:FindFirstChild("ActiveIceShard")
    end
    if not Shard and Learn then
        Shard = Learn:FindFirstChild("ActiveIceShard")
    end
    if not Shard then
        Notify("Vendetta Hub", "Active Ice Shard is not loaded yet.", 3)
        return
    end

    local TargetPosition
    if Shard:IsA("BasePart") then
        TargetPosition = Shard.Position
    elseif Shard:IsA("Model") then
        TargetPosition = Shard:GetPivot().Position
    else
        local Part = Shard:FindFirstChildWhichIsA("BasePart", true)
        if Part then
    ...
    if not TargetPosition then
        Notify("Vendetta Hub", "Active Ice Shard has no valid position.", 3)
        return
    end
    ...
    if IceShardTween then
        pcall(function() IceShardTween:Cancel() end)
        IceShardTween = nil
    end
...
local function GetIceShard()
    -- Only use the known Pure Ice Man hierarchy; never scan Workspace.
    local ChatNPCs = workspace:FindFirstChild("chatnpcs")
    local PureIceMan = ChatNPCs and ChatNPCs:FindFirstChild("Pure Ice Man")
    -- Supported locations:
    -- Workspace > chatnpcs > Pure Ice Man > ActiveIceShard
    ...
    local Learn = Chat:FindFirstChild("Learn")
    return Learn and Learn:FindFirstChild("ActiveIceShard") or nil
end
...
local function GetIceShardPart(Shard)
    ...
    return Shard:FindFirstChildWhichIsA("BasePart", true)
end
...
--// OP
--// OP BUTTONS
local SprintBypassButton = Instance.new("TextButton")
SprintBypassButton.Name = "SprintBypassButton"
SprintBypassButton.Text = "Bypass"
SprintBypassButton.Size = UDim2.new(1, -10, 0, 36)
SprintBypassButton.Position = UDim2.new(0, 5, 0, 140)
SprintBypassButton.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
SprintBypassButton.TextColor3 = Color3.fromRGB(255, 255, 255)
SprintBypassButton.BorderSizePixel = 0
SprintBypassButton.BackgroundTransparency = 0
SprintBypassButton.Parent = OP
local SprintBypassButton_Corner = Instance.new("UICorner")
SprintBypassButton_Corner.CornerRadius = UDim.new(0, 6)
SprintBypassButton_Corner.Parent = SprintBypassButton


SprintBypassButton.MouseButton1Click:Connect(function()
    if SprintBypassEnabled then
        StopSprintBypass()
        SprintBypassButton.Text = "Bypass"
    else
        StartSprintBypass()
        SprintBypassButton.Text = "Bypass [ON]"
    end
end)

local AntiFallDmgButton = Instance.new("TextButton")
AntiFallDmgButton.Name = "AntiFallDmgButton"
AntiFallDmgButton.Text = "Anti Fall Dmg"
AntiFallDmgButton.Size = UDim2.new(1, -10, 0, 36)
AntiFallDmgButton.Position = UDim2.new(0, 5, 0, 182)
AntiFallDmgButton.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
AntiFallDmgButton.TextColor3 = Color3.fromRGB(255, 255, 255)
AntiFallDmgButton.BorderSizePixel = 0
AntiFallDmgButton.BackgroundTransparency = 0
AntiFallDmgButton.Parent = OP
local AntiFallDmgButton_Corner = Instance.new("UICorner")
AntiFallDmgButton_Corner.CornerRadius = UDim.new(0, 6)
AntiFallDmgButton_Corner.Parent = AntiFallDmgButton


AntiFallDmgButton.MouseButton1Click:Connect(function()
    if AntiFallDmgEnabled then
        StopAntiFallDmg()
        AntiFallDmgButton.Text = "Anti Fall Dmg"
    else
        StartAntiFallDmg()
        AntiFallDmgButton.Text = "Anti Fall Dmg [ON]"
    end
end)
--==================================================
...
--==================================================
--// ACQUISITION ADDON
--// Deliberately deferred so an addon error cannot stop the original hub.
--==================================================

task.defer(function()
    local ok, err = pcall(function()

        local KatanaSellerPosition = Vector3.new(
            -251.04994201660156,
            821.5532836914062,
            -30.124601364135742
        )
        ...
        local function desyncFor(seconds)
            stopDesync()
            ...
            task.delay(seconds, stopDesync)
        end

        -- Finds a ProximityPrompt reliably even when the prompt is nested
        -- inside a model/folder (for example: Workspace.Katana.ProximityPrompt).
        local function findTarget(name)
            ...
            -- Small aliases used by different versions of the game.
            local aliases = {
                ["Katana Seller"] = {"Katana", "KatanaSeller", "Katana Seller"},
                ["Old Akuma"] = {"Old Akuma", "OldAkuma"},
            }
        ...
        local function findPrompt(root)
            ...
            -- Prefer the actual named ProximityPrompt shown in the Katana
            -- object instead of an unrelated prompt nearby.
            local direct = root:FindFirstChild("ProximityPrompt", true)
        ...
        -- Old Akuma-style prompt firing used by the older working versions.
        -- Keep this deliberately simple: get the ProximityPrompt and call
        -- fireproximityprompt directly, with no alternate firing path.
        ...
        local function usePrompt(name)
            local target = findTarget(name)
            local prompt = findPrompt(target)

            -- The Katana seller is shown as Workspace.Katana in the game.
            -- Keep the public name "Katana Seller" for the rest of the addon.
            if not prompt and name == "Katana Seller" then
                local katana = workspace:FindFirstChild("Katana", true)
                prompt = findPrompt(katana)
            end

            if not prompt then
                warn("[VENDETTA ACQUISITION] No ProximityPrompt found for " .. tostring(name))
                return false
            end

            -- Katana and every trainer intentionally use the same old
            -- Akuma prompt-firing method.
            local fired = firePrompt(prompt)
            if not fired then
                warn("[VENDETTA ACQUISITION] Failed to fire ProximityPrompt for " .. tostring(name))
            end
            return fired
        end

        local function moveTo(position)
            -- Use the already-existing Vendetta tween routine.
        ...
        -- Simple self-contained draggable selector. It does not depend on
        -- MakeDraggable, so the addon has fewer dependencies on the hub.
        local selector
        local TeleportBreathingMode = false
        ...
        local function openSelector()
            ...
            selector.Name = "VendettaBreathingSelector"
            selector.Size = UDim2.fromOffset(270, 450)
            selector.Position = UDim2.new(.5, -135, .5, -225)
            selector.BackgroundColor3 = Color3.fromRGB(11,11,17)
            ...
            local header = Instance.new("TextButton", selector)
            header.Size = UDim2.new(1,-50,0,45)
            header.Position = UDim2.fromOffset(10,5)
            header.BackgroundTransparency = 1
            header.Text = "BREATHING TRAINERS"
            ...
            local close = Instance.new("TextButton", selector)
            ...
            local modeButton = Instance.new("TextButton", selector)
            modeButton.Size = UDim2.new(1,-20,0,32)
            modeButton.Position = UDim2.fromOffset(10,55)
            ...
            local function refreshModeButton()
                modeButton.Text = "MOVEMENT: " .. (TeleportBreathingMode and "TELEPORT" or "TWEEN")
                modeButton.TextColor3 = TeleportBreathingMode and Color3.fromRGB(235,45,45) or Color3.new(1,1,1)
            end
            refreshModeButton()

            modeButton.MouseButton1Click:Connect(function()
                TeleportBreathingMode = not TeleportBreathingMode
                refreshModeButton()
            end)

            local scroll = Instance.new("ScrollingFrame", selector)
            scroll.Size = UDim2.new(1,-20,1,-100)
            scroll.Position = UDim2.fromOffset(10,95)
            ...
            for _, trainerName in ipairs(TrainerOrder) do
                local button = Instance.new("TextButton",scroll)
                button.Size = UDim2.new(1,-4,0,38)
                button.BackgroundColor3 = Color3.fromRGB(19,20,27)
                button.BorderSizePixel = 0
                button.Text = trainerName
                ...
                button.MouseButton1Click:Connect(function()
                    ...
                    task.spawn(function()
                        local position = TrainerPoints[trainerName]
                        if not position then return end

                        -- Phase 1: tween all the way to the Katana Seller before doing anything else.
                        local function tweenAndWait(targetPosition, duration)
                            local char = LP.Character
                        ...
                        if TeleportBreathingMode then
                            -- Teleport mode: Katana -> wait 1s -> desync 1s -> trainer -> prompt.
                            local char = LP.Character
                            local root = char and char:FindFirstChild("HumanoidRootPart")
                            if not root then return end
                            ...
                            root.CFrame = CFrame.new(KatanaSellerPosition)
                            task.wait(1)
                            usePrompt("Katana Seller")
                            ...
                            task.wait(.1)
                            usePrompt(trainerName)
                        else
                            -- Tween mode: use the existing timing.
                            if not tweenAndWait(KatanaSellerPosition, 3) then return end
                            task.wait(.1)
                            usePrompt("Katana Seller")

                            -- Desync for 1 second total. After 0.5 seconds of desync,
                            -- start the trainer tween while the desync is still running.
                            desyncFor(1)
                            task.wait(.5)

                            -- Close trainers use 0.5s; farther trainers use 1s.
                            local FastTrainers = {
                                ["Rock Trainer"] = true,
                                ["Water Trainer"] = true,
                                ["Mist Trainer"] = true,
                            }
                            local TrainerDuration = FastTrainers[trainerName] and 0.5 or 1

                            if not tweenAndWait(position, TrainerDuration) then return end
                            task.wait(.1)
                            stopDesync()
                            usePrompt(trainerName)
                        end
        ...
        AutoBreathingsButton.MouseButton1Click:Connect(openSelector)

        AutoBDAsButton.MouseButton1Click:Connect(function()
            task.spawn(function()
                local target = findTarget("Old Akuma")
                local part = target and (
                    target:IsA("BasePart") and target
                    or target:IsA("Model") and (target.PrimaryPart or target:FindFirstChildWhichIsA("BasePart",true))
                )

                if not part then
                    warn("[VENDETTA ACQUISITION] Old Akuma not found.")
                    return
                end
                ...
                -- Burst 2: 2 seconds. No second prompt.
                desyncFor(2)
            end)
        end)

        print("[VENDETTA ACQUISITION] addon installed")

    end)

    if not ok then
        warn("[VENDETTA ACQUISITION] addon failed without stopping Vendetta:")
        warn(tostring(err))
    end
end)
